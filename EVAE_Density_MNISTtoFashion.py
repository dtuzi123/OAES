...
testingSet = np.concatenate((Xtest,fashion_Xtest),axis=0)
        ...
        with tf.Session(config=config) as sess:
            ...
            for index in range(totalCount):
                ...
                for epoch2 in range(5):
                    Xtrain1 = self.FixedMemory
                    myCount = int(np.shape(Xtrain1)[0] / self.batch_size)

                    n_examples = np.shape(Xtrain1)[0]
                    index2 = [i for i in range(n_examples)]
                    np.random.shuffle(index2)
                    Xtrain1 = Xtrain1[index2]

                    for idx in range(myCount):
                        batchImages = Xtrain1[idx*self.batch_size:(idx+1)*self.batch_size]

                        batch_z = np.random.uniform(-1, 1, [self.batch_size, self.z_dim]).astype(np.float32)

                        _, d_loss = self.sess.run([self.d_optim, self.d_loss],
                                                  feed_dict={self.inputs: batchImages, self.z: batch_z})
                ...
                for epoch in range(epochs):
                    ...
                    if epoch % 2 == 0:
                        print("epoch {0}/{1}, step {2}/{3}, train ELBO: {4:.2f}, val ELBO: {5:.2f}, time: {6:.2f}"
                                  .format(d_loss, g_loss, index, totalCount, d_loss, np.shape(self.lossArr)[0], 0))
                    ...
                    if self.componentCount > 1:
                        for kk in range(self.componentCount - 1):
                            ...
                            for i in range(mycount2):
                                ...
                                gen1 = self.sess.run(self.GeneratorArr[kk], feed_dict={self.z: batch_z})
                                gen2 = self.sess.run(self.GeneratorArr[np.shape(self.GeneratorArr)[0] - 1],
                                                     feed_dict={self.z: batch_z})

                                gen1 = np.reshape(gen1,(self.batch_size,28*28))
                                gen2 = np.reshape(gen2,(self.batch_size,28*28))
                                score = self.sess.run(self.evaluationLoss,
                                                      feed_dict={self.inputs: gen1, self.gan_inputs: gen2})
